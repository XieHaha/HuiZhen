package com.yht.frame.widgets.picker.wheelview.view;
...
import com.yht.frame.widgets.picker.wheelview.adapter.WheelAdapter;
import com.yht.frame.widgets.picker.wheelview.interfaces.IPickerViewData;
import com.yht.frame.widgets.picker.wheelview.listener.LoopViewGestureListener;
import com.yht.frame.widgets.picker.wheelview.listener.OnItemSelectedListener;
import com.yht.frame.widgets.picker.wheelview.timer.InertiaTimerTask;
import com.yht.frame.widgets.picker.wheelview.timer.MessageHandler;
import com.yht.frame.widgets.picker.wheelview.timer.SmoothScrollTimerTask;
...
public class WheelView extends View {
    ...
    public WheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        textSize = getResources().getDimensionPixelSize(R.dimen.text_18);//默认大小
        ...
        if (attrs != null) {
            ...
            lineSpacingMultiplier = a.getFloat(R.styleable.pickerview_wheelview_lineSpacingMultiplier,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (!isCenterLabel && !TextUtils.isEmpty(label) &&
                    !TextUtils.isEmpty(getContentText(visibles[counter]))) {
                ...
                float translateY = (float)(radius - Math.cos(radian) * radius -
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.scale(1.0F, (float)Math.sin(radian) * SCALE_CONTENT);
                    // 控制文字倾斜角度
                    paintOuterText.setTextSkewX(
                            (textXOffset == 0 ? 0 : (textXOffset > 0 ? 1 : -1)) * (angle > 0 ? -1 : 1) *
                            DEFAULT_TEXT_TARGET_SKEW_X * offsetCoefficient);
                    // 控制透明度
                    paintOuterText.setAlpha((int)((1 - offsetCoefficient) * 255));
                    // 控制文字水平偏移距离
                    canvas.drawText(contentText, drawOutContentStart + textXOffset * offsetCoefficient, maxTextHeight,
                                    paintOuterText);
                    canvas.restore();
                }
                canvas.restore();
