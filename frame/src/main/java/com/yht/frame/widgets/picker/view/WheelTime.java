package com.yht.frame.widgets.picker.view;

import android.view.Gravity;
import android.view.View;

import com.yht.frame.R;
import com.yht.frame.widgets.picker.adapter.ArrayWheelAdapter;
import com.yht.frame.widgets.picker.adapter.NumericWheelAdapter;
import com.yht.frame.widgets.picker.adapter.NumericWheelMonthAdapter;
import com.yht.frame.widgets.picker.listener.ISelectTimeCallback;
import com.yht.frame.widgets.picker.utils.ChinaDate;
import com.yht.frame.widgets.picker.utils.LunarCalendar;
import com.yht.frame.widgets.picker.wheelview.listener.OnItemSelectedListener;
import com.yht.frame.widgets.picker.wheelview.view.WheelView;
...
public class WheelTime {
    ...
    private int startHour = DEFAULT_start_HOUR;//时间
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        wv_month.setGravity(Gravity.RIGHT);
        // 日
        ...
        wv_hours.setGravity(Gravity.START);
        ...
        wv_year.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                if (ChinaDate.leapMonth(year_num) != 0 &&
                    wv_month.getCurrentItem() > ChinaDate.leapMonth(year_num) - 1) {
                ...
                if (ChinaDate.leapMonth(year_num) != 0 &&
                    ...
                    if (wv_month.getCurrentItem() == ChinaDate.leapMonth(year_num) + 1) {
                        ...
                        wv_day.setAdapter(new ArrayWheelAdapter(
                                ChinaDate.getLunarDays(ChinaDate.monthDays(year_num, wv_month.getCurrentItem()))));
                        maxItem = ChinaDate.monthDays(year_num, wv_month.getCurrentItem());
                    }
                }
                else {
                    wv_day.setAdapter(new ArrayWheelAdapter(
                            ChinaDate.getLunarDays(ChinaDate.monthDays(year_num, wv_month.getCurrentItem() + 1))));
            ...
            public void onItemSelected(int index) {
                ...
                if (ChinaDate.leapMonth(year_num) != 0 && month_num > ChinaDate.leapMonth(year_num) - 1) {
                    if (wv_month.getCurrentItem() == ChinaDate.leapMonth(year_num) + 1) {
                        ...
                        wv_day.setAdapter(new ArrayWheelAdapter(
                                ChinaDate.getLunarDays(ChinaDate.monthDays(year_num, month_num))));
                        maxItem = ChinaDate.monthDays(year_num, month_num);
                    }
                }
                else {
                    wv_day.setAdapter(new ArrayWheelAdapter(
                            ChinaDate.getLunarDays(ChinaDate.monthDays(year_num, month_num + 1))));
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        if (startYear == endYear) {//开始年等于终止年
            wv_month.setAdapter(new NumericWheelMonthAdapter(startMonth, endMonth));
            wv_month.setCurrentItem(month + 1 - startMonth);
        }
        else if (year == startYear) {
            //起始日期的月份控制
            wv_month.setAdapter(new NumericWheelMonthAdapter(startMonth, 12));
            wv_month.setCurrentItem(month + 1 - startMonth);
        }
        else if (year == endYear) {
            //终止日期的月份控制
            wv_month.setAdapter(new NumericWheelMonthAdapter(1, endMonth));
            wv_month.setCurrentItem(month);
        }
        else {
            wv_month.setAdapter(new NumericWheelMonthAdapter(1, 12));
            wv_month.setCurrentItem(month);
        }
        wv_month.setGravity(Gravity.RIGHT);
        // 日
        ...
        wv_day.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                if (currentMonth == startMonth && index == 0) {
                    wv_hours.setAdapter(new NumericWheelAdapter(startHour, 23));
                }
                else {
                    wv_hours.setAdapter(new NumericWheelAdapter(0, 23));
    ...
    private void setReDay(int year_num, int monthNum, int startD, int endD, List<String> list_big,
    ...
    public void setLabels(String label_year, String label_month, String label_day, String label_hours,
            String label_mins, String label_seconds) {
        if (isLunarCalendar) {
            return;
        }
        if (label_year != null) {
    ...
    public void setTextXOffset(int x_offset_year, int x_offset_month, int x_offset_day, int x_offset_hours,
           ...
           /* int i = wv_month.getCurrentItem() + startMonth;
            ...
            if ((wv_month.getCurrentItem() + startMonth) == startMonth) {
                ...
                if (wv_day.getCurrentItem() == 0) {
                    sb.append(wv_hours.getCurrentItem() + startHour);
                }
                else {
                    sb.append(wv_hours.getCurrentItem());
                }
                sb.append(":").append(wv_minutes.getCurrentItem()).append(":").append(wv_seconds.getCurrentItem());
            }
            else {
                sb.append((wv_year.getCurrentItem() + startYear))
                  .append("-")
